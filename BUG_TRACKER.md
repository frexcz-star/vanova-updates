...
| BUG-017 | MEDIUM | UX | 3.0.8 | QA-PARTIAL → READY FOR RETEST | `web/dashboard.html`, `web/index.html`, `web/dist/*`, `desktop/runtime/opportunity_catalog.py`, `desktop/runtime/api_server.py`, `desktop/runtime/runtime_security.py`, `web/data-services.js` | `test_opportunity_catalog.py::test_bug017_mark_done_connects_to_action_loop` | SPEC-OPP: la vista "Oportunidades" (spec §4.1) NO estaba integrada en la interfaz. Fix: sección "Oportunidades de crecimiento" en la vista Insights (`viewInsights`), con tarjetas (`opportunityCard`) que muestran `upsideEuro` en € grande cuando hay evidencia y "Impacto no cuantificable" cuando es None (nunca 0). Copy ES corporate, sin emojis. Empty state honesto. `getOpportunities()` ahora se llama en `loadAppData` (store.opportunities). **CTA "Marcar como hecha"** (spec §4.1 → action-loop): botón por tarjeta que llama `opportunity_catalog.mark_done` (registra la oportunidad como recomendación por firma estable + la marca done → dispara medición), endpoint POST `/api/opportunities/done`, whitelist de mutación actualizado. Sincronizado a web/index.html + web/dist/. Suite: 701 passed. **Review Mathew (2026-08-20):** código del repo CORRECTO (vista, €, "no cuantificable", empty state, escape, copy). PERO build-desync: el servidor en vivo sirve la build ANTIGUA (hash 9f36e110) SIN la vista; `release/win-unpacked` e instalación real (`AppData/Local/Programs/VANOVA`) siguen con `9f36e110`, solo el repo tiene `c5806f8f`. Mismo patrón BUG-003-BUILD/BUG-005. REQUIRE: sincronizar build empaquetada + instalación + E2E con el repo antes de CLOSED. |
