...
| BUG-027 | MEDIUM | Hermes | 3.0.9 | FIXED → READY FOR RETEST | `desktop/runtime/important_store.py` | `tests/test_important_store.py::test_unmark_uses_atomic_update` | **RMW no atómico en `important_store.unmark`.** Hacía `_load()` → modificar lista → `_save()` sin `config_store.update()`, mismo patrón lost-update de BUG-006/015/019/023. Expuesto vía `POST /api/important/unmark` (ThreadingHTTPServer): un mark concurrente podía reintroducir el item eliminado o perderse. Fix (Hermes, 2026-08-21): refactorizado a `config_store.update(_mutate)` (RMW atómico), coherente con `mark_important`. Regression test verifica que `unmark` usa `update()`. Suite: 726 passed. |
| BUG-028 | MEDIUM | Archivos | 3.0.9 | FIXED → READY FOR RETEST | `desktop/runtime/file_inventory.py`, `desktop/runtime/business_scanner.py` | `tests/test_bug017.py::Bug028FileRemovalPersistsTests` | **Persistencia de Archivos: al eliminar (o aceptar/agregar) un archivo el cambio no sobrevivía al reiniciar VANOVA.** Root cause: `remove_imported_file` solo quitaba el path de `scanFiles`, pero el archivo seguía en disco; el siguiente scan (`_scan_files` → `_save_scan_files`) lo **reintroducía** en `scanFiles`. Fix (Hermes, 2026-08-21): (1) `remove_imported_file` registra el path en `scanExclusions` (RMW atómico); (2) `_scan_files` respeta `scanExclusions` y no vuelve a añadir archivos excluidos. Regression tests: verifica que remove añade la exclusión y que el scan la respeta. Suite: 728 passed. |
| BUG-029 | HIGH | Hermes | 3.0.9 | FIXED → READY FOR RETEST | `desktop/runtime/agent_architect.py` | `tests/test_agent_scheduler.py::Bug029AgentRoleAndBotSyncTests` | **Agente sin bot Hermes persistente (`hermesBot=None`, `role=None`).** Root cause doble: (1) `_normalize_agent` descartaba `role` y `hermesBot` al guardar; (2) `add_agents` (usado por `/api/agents/add` del catálogo) NO sincronizaba al bot Hermes — solo `create_custom_agent` lo hacía. Así el agente quedaba sin perfil bot persistente pese a que la UI lo presentaba como tal ("no conversations yet"). Fix (Hermes, 2026-08-21): (1) `_normalize_agent` preserva `role`/`hermesBot`; (2) `add_agents` sincroniza cada agente nuevo a un bot Hermes (`_sync_added_agents_to_bots` → `sync_agent_to_bot` + persiste `hermesBot` + rutina cron); (3) `create_custom_agent` reporta honestamente "Bot de Hermes no disponible" si el sync falla (en vez de silencio). Tests parchean el sync para no crear perfiles reales en la suite. Suite: 730 passed. |
