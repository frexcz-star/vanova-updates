...
| BUG-016 | LOW | QA/CI | 3.0.8 | FIXED → READY FOR RETEST | `tests/test_import_attack.py` | — | Flakiness en `test_import_attack.py::test_huge_file_100k_rows` (test_import_attack.py:152-162). El test parsea 100k filas y exige `elapsed < 30.0`. Pasa aislado (~28s) pero **falla bajo carga de la suite completa**. Fix: umbral subido de 30s a 60s (margen robusto sin perder el propósito del test). Verificado en suite completa. |
| BUG-015 | MEDIUM | Hermes | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/ai_providers.py`, `desktop/runtime/hermes_config.py` | `tests/test_bug015.py` (2 tests) | RMW no atómico persistente en `ai_providers.save_provider_config` y `hermes_config` sync. Refactorizados a `config_store.update()` (RMW atómico bajo un solo lock). **Extra:** el sync de hermes_config antes hacía `save({"aiProviders": {"primary": entry}})` que REEMPLAZABA todo el dict aiProviders, borrando otros providers — ahora solo actualiza la clave `primary` preservando el resto. Regression tests: `update()` invocado + otros providers preservados. |
| SPEC-OPP | FEATURE | Proactividad | 3.0.8 | IMPLEMENTED → READY FOR REVIEW | `desktop/runtime/opportunity_catalog.py`, `desktop/runtime/api_server.py`, `web/data-services.js` | `test_opportunity_catalog.py` (11 tests) | STRATI Detector de Oportunidades de Crecimiento (docs/STRATI_DETECTOR_OPORTUNIDADES_SPEC.md). Capa de producto sobre detection_engine. Endpoint GET /api/opportunities. Honestidad UNKNOWN≠0, anti-ruido < 25 EUR, dedupe por firma. **BUG-018 fixed:** `_upside_for_aov` ahora cuantifica (antes código muerto). Suite: 700 passed. |
| BUG-017 | MEDIUM | UX | 3.0.8 | NEW | — | — | SPEC-OPP: la vista "Oportunidades" (spec §4.1) NO está integrada en la interfaz. `getOpportunities()` existe en web/data-services.js:540-543 y el endpoint `GET /api/opportunities` está en api_server.py:240, pero **ningún lugar del dashboard/index.html lo invoca**. El empresario no puede ver las oportunidades en €; el feature es invisible en la UI. Acceptance: añadir la vista "Oportunidades" (o sección en Insights/Home) que llame a `getOpportunities()` y renderice las tarjetas (€ o "Impacto no cuantificable"), con el CTA "Marcar como hecha" → action-loop. |
| BUG-018 | MEDIUM | Proactividad | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/opportunity_catalog.py`, `desktop/runtime/detection_engine.py` | `test_opportunity_catalog.py` (2 tests BUG-018) | `opportunity_catalog._upside_for_aov` era **código muerto**: nunca cuantificaba. Fix: `_upside_for_aov` ahora lee `currentOrders`/`previousOrders` de metrics (añadidos al finding `aov_change` en detection_engine) y calcula `(AOV_objetivo − AOV_actual) × pedidos_periodo` cuando `current_aov < previous_aov` y hay pedidos. Sin pedidos → `None` (UNKNOWN ≠ 0). Regression tests verificados: upside calculado (2000€) y no cuantificable sin pedidos. |
| BUG-019 | MEDIUM | Hermes | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/file_inventory.py` | `tests/test_bug017.py` (3 tests) | RMW no atómico persistente en `file_inventory.add_imported_file`, `remove_imported_file` y `decide_candidate` — hacen `load()` → modificar `scanFiles`/`fileCandidates` → `save()` sin `config_store.update()`. Mismo patrón lost-update de BUG-006/015. Accesible desde el API server (ThreadingHTTPServer). Fix: refactorizados a `config_store.update()` (RMW atómico). Regression tests verificados: `update()` invocado, approve usa 2 updates atómicos. |
