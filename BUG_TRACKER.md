...
| BUG-034 | HIGH | Shopify | 3.1.1 | FIXED → READY FOR RETEST | `desktop/runtime/shopify_sync.py` | `tests/test_shopify_backfill.py::Bug034AtomicShopifyRmwTests` | **RMW no atómico en el backfill de Shopify (pérdida de datos concurrente).** `backfill_line_items` y `recover_variant_identity` hacían fetch de red largo (sleep 0.25s/pedido, puede durar minutos) acumulando la lista en memoria y al final `config_store.save({"organizedSales"/"organizedProducts": ...})` SOBRESCRIBÍA la lista completa. Si otro escritor (cost_importer, facturascripts_sync, un sync de Shopify simultáneo) persistía esas claves durante el fetch, su cambio se perdía (lost-update, mismo patrón que BUG-006/015/019/021/027). Fix (Hermes, 2026-08-21): ambos ahora persisten con `config_store.update()` capturando solo los registros actualizados (pedidos con line_items / productos con variantId) y re-aplicándolos sobre la lista vigente, sin pisar escrituras concurrentes. Regression tests: verifica que usan `update()` y no `save()`. Suite: 738 passed. |
