...
| BUG-035 | MEDIUM | Startup | 3.1.1 | FIXED → READY FOR RETEST | `desktop/runtime/*` sincronizado del repo → instalación real | — | **Desync de backend: el fix de BUG-031 (regeneración de contraseña débil del cloud) NO estaba en la instalación real del usuario.** Root cause: el runtime completo de la instalación real (`AppData/Local/Programs/VANOVA/resources/vanova/desktop/runtime/`) estaba desyncado del repo — 45 .py con hash distinto, incluyendo `process_manager.py` (hash `e969203` sin fix) vs repo (`c80a1d3`/`4e147f39` con `_is_weak_password`/`KNOWN_WEAK_PASSWORDS`). Impacto: un usuario con la contraseña débil por defecto (`mooving2026`) vería "cloud start failed". Fix (Hermes, 2026-08-21): sincronizado el runtime completo del repo → instalación real (45 archivos, 0 desync restantes); verificado `process_manager.py` en instalación con hash `4e147f39` y fix presente (5 apariciones `_is_weak_password`/`KNOWN_WEAK_PASSWORDS`); runtime relanzado desde la instalación real → `/api/health` HTTP 200, cloud :8000 HTTP 200. |
