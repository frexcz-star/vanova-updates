...
| BUG-031 | HIGH | Cloud | 3.1.1 | FIXED → READY FOR RETEST | `desktop/runtime/process_manager.py` | `tests/test_production_hardening.py::Bug031WeakPasswordAutoRegenTests` | **"cloud start failed" al arrancar con la contraseña débil por defecto del instalador.** El instalador desplegaba `cloud.env` con `MAIOS_DEMO_PASSWORD=mooving2026`; al añadir `KNOWN_WEAK_PASSWORDS` al bootstrap del cloud, cualquier cliente que instale la build nueva bloquea el arranque en producción. Root cause: el despliegue (`_ensure_env_files`) dejaba la contraseña débil y el cloud la rechazaba. Fix (Hermes, 2026-08-21): `_ensure_env_files` regenera automáticamente la contraseña con `secrets.token_urlsafe(16)` si la existente es débil/por defecto (`_is_weak_password`), en vez de bloquear. Idempotente: una contraseña fuerte existente NO se regenera. Regression test: con `mooving2026` en cloud.env, `_ensure_env_files` la reemplaza por una fuerte. Suite: 732 passed. |
