...
| BUG-010 | HIGH | Cloud | 3.0.7 | FIXED → READY FOR RETEST | `cloud/main.py` | `test_bug010_snapshots_pruned_to_retention` + `test_bug010_different_kinds_kept_separate` | Crecimiento infinito de la tabla `snapshots` en el cloud (SQLite). `store_snapshot` hacía `INSERT` en CADA llamada sin ningún DELETE/prune/retention. Fix: constante `SNAPSHOT_RETENTION = 100` y prune al insertar que conserva solo los últimos 100 por (workspace_id, kind) usando el rowid implícito. Los SELECT usan `ORDER BY rowid DESC LIMIT 1`, así que no cambia ningún consumidor. Regression test verificado: tras insertar retention+20, quedan ≤100 y el último es el más reciente. |
| BUG-011 | MEDIUM | Hermes | 3.0.7 | FIXED → READY FOR RETEST | `desktop/runtime/hermes_activity.py` | `test_bug011_log_step_uses_atomic_update` + `test_bug011_log_accumulates_and_trims` | RMW no atómico persistente en `hermes_activity.log_step`. Refactorizado a `config_store.update()` (RMW atómico bajo un solo lock) — igual que BUG-006/008. Regression tests verificados: `update()` invocado, el log acumula y trima a MAX_LOG. |
| BUG-012 | MEDIUM | Hermes | 3.0.7 | FIXED → READY FOR RETEST | `desktop/runtime/file_organizer.py` | `test_bug012_add_product_uses_atomic_update` + `test_bug012_add_product_persists` | RMW no atómico persistente en `file_organizer.add_product`. Refactorizado a `config_store.update()` (RMW atómico). Regression tests verificados: `update()` invocado, ambos productos persisten. |
| BUG-013 | HIGH | Cloud | 3.0.8 | FIXED → READY FOR RETEST | `cloud/main.py` | `test_bug010_different_kinds_kept_separate` | Regresión de pérdida de datos del fix de BUG-010: el DELETE del prune solo filtraba en el sub-SELECT, borrando filas de TODOS los kinds/workspaces. Fix: `WHERE workspace_id=? AND kind=? AND rowid NOT IN (...)` en el DELETE. Regression test verificado: falla con el código sin fix (1 != 2), pasa con el fix (2 filas preservadas por par). |
| BUG-014 | LOW | QA/CI | 3.0.8 | FIXED → READY FOR RETEST | `tests/test_bug010_011_012.py` | — | Error de tipado en `setUp` de `Bug010SnapshotRetentionTests`: asignaba `self._patch_db` pero llamaba `self._patch.start()`. Fix: renombrado a `_get_db_patcher.start()` / `_get_db_patcher.stop()`, aislando la DB en un archivo temporal (no toca `maios_cloud.db`). Verificado: 6 tests de la clase pasan. |
