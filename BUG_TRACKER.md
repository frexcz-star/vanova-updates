...
| BUG-020 | MEDIUM | Proactividad | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/opportunity_catalog.py` | `tests/test_opportunity_catalog.py::test_cross_sell_sku_case_insensitive_margin` | Los SKUs del par cross-sell llegan en minúsculas (`box-501`) pero el catálogo en mayúsculas (`BOX-501`): el margen no se encontraba y el upside del cross-sell NUNCA se cuantificaba (siempre `None`/no_quantifiable). Fix: normalizar a lowercase en el match SKU↔catálogo. Regression test: con SKU en minúsculas y coste verificado, el upside ahora se cuantifica. Suite: 702 passed. |
