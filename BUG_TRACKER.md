...
| BUG-008 | MEDIUM | Hermes | 3.0.7 | FIXED → READY FOR RETEST | `desktop/runtime/detection_engine.py` | `test_bug008_update_finding_status_uses_atomic_rmw` | Lost-update en `update_finding_status` (mismo patrón que BUG-006). Root cause: hacía `load()` → modificar → `save()` sin serializar el RMW completo; con ThreadingHTTPServer (API server) y el scheduler en background escribiendo `businessFindings` a la vez, dos hilos podían perder el estado del finding. Fix: refactorizado para usar `config_store.update()` (RMW atómico bajo un solo lock). Regression test verificado: `update()` invocado, estado persiste. |
| BUG-009 | LOW | Connector | 3.0.7 | FIXED → READY FOR RETEST | `connector/connector.py` | — | Código muerto en carga de env del connector: `_user_connector_env` y `_legacy_connector_env` eran la MISMA ruta (`LOCALAPPDATA/VANOVA/config/connector.env`), así que el `elif` nunca se ejecutaba. No es bug funcional (el fallback era redundante), pero es defecto de calidad. Fix: unificada en una sola ruta de config del usuario. |
