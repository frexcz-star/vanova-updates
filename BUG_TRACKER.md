...
| ID | Severidad | Área | Versión | Estado | Fix en | Regression test | Notas |
|----|-----------|------|---------|--------|--------|-----------------|-------|
| BUG-023 | MEDIUM | Hermes | 3.0.8 | NEW | — | — | RMW no atómico persistente en `product_identity` (`add_mapping`, `remove_mapping`, `ignore_sku`, `unignore_sku`). Hacen `load()` → modificar lista → `save()` sin `config_store.update()` (product_identity.py:158-185, 190-195, 215-223, 226-231). Expuestos en `/api/products/match`, `/api/products/match/remove`, `/api/products/ignore`, `/api/products/ignore/remove` (api_server.py:696-736, ThreadingHTTPServer). Mismo patrón lost-update de BUG-006/015/019: dos requests concurrentes pueden perder un mapping/SKU. Fix: `config_store.update()`. Reproducibilidad 2/5. _(Nota: este bug estaba duplicado como BUG-021 en dos rondas — se renombra a BUG-023 para distinguirlo del BUG-021 de agent_scheduler, que está CLOSED.)_ |
