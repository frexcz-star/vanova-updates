...
| BUG-030 | HIGH | Hermes | 3.0.9 | FIXED → READY FOR RETEST | `desktop/runtime/hermes_chat.py`, `desktop/runtime/task_queue.py` | `tests/test_task_execution.py::test_execute_task_passes_agent_bot_profile` | **Tarea del agente no persiste como conversación en su perfil Hermes.** Root cause: `task_queue._execute_task` llamaba `hermes_chat.execute_sync(message)` sin perfil → `_run_hermes_cli` construía `cmd = <hermes> chat -q …` (SIN `-p`), ejecutando en el perfil por defecto de Hermes. El bot del agente (`vanova-sales-analyst`) existía pero no recibía la conversación → "no conversations yet". Fix (Hermes, 2026-08-21): (1) `task_queue._execute_task` lee `agent.hermesBot` y lo pasa a `execute_sync(message, profile=…)` (también en el retry por datos); (2) `hermes_chat.execute_sync` → `_run_chat_with_slot` → `_run_hermes_cli` aceptan `profile` y añaden `hermes -p <profile> chat -q …` al comando. Así la conversación queda persistida en el perfil del bot del agente (bot-mode). Regression test: verifica que `_execute_task` pasa el perfil del bot. Suite: 731 passed. |
