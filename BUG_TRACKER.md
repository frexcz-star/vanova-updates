...
| ID | Severidad | Área | Versión | Estado | Fix en | Regression test | Notas |
|----|-----------|------|---------|--------|--------|-----------------|-------|
| BUG-023 | MEDIUM | Hermes | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/product_identity.py` | `tests/test_product_identity.py::Bug023AtomicRmwTests` | RMW no atómico persistente en `product_identity` (`add_mapping`, `remove_mapping`, `ignore_sku`, `unignore_sku`). Hacen `load()` → modificar lista → `save()` sin `config_store.update()` (product_identity.py:158-185, 190-195, 215-223, 226-231). Expuestos en `/api/products/match`, `/api/products/match/remove`, `/api/products/ignore`, `/api/products/ignore/remove` (api_server.py:696-736, ThreadingHTTPServer). Mismo patrón lost-update de BUG-006/015/019: dos requests concurrentes pueden perder un mapping/SKU. Fix: refactorizados los 4 a `config_store.update()` (RMW atómico bajo un solo lock; se conserva `updated`/`removed`/`unignored` con `nonlocal`). Regression tests: `Bug023AtomicMutationTests` (add_mapping e ignore_sku usan update). Helpers `_patch_all` de test_product_identity/test_decoupling actualizados para parchear `update` también. Suite: 707 passed. _(Nota: este bug estaba duplicado como BUG-021 en dos rondas — se renombra a BUG-023 para distinguirlo del BUG-021 de agent_scheduler, que está CLOSED.)_ |
| BUG-022 | MEDIUM | Shopify | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/integrations_store.py` | `tests/test_hermes_shopify_setup.py::ClearStaleShopifyErrorsTests` | `integrations_store.clear_stale_shopify_sync_errors` (integrations_store.py:134-152) reemplaza TODO el objeto `shopifySync` por un sub-dict de solo 6 campos (`missingScopes`, `scopeErrors`, `errorCategory`, `lastError`, `userMessage`, `partial`), perdiendo `lastSync`, `counts`, `status`, `message`, `guardAlerts`, `guard`, `backfill`, `startedAt`. Se llama desde `sync_shopify_from_hermes_if_needed` (L210, 235) cuando las credenciales validan. Consecuencia: al limpiar un error de permisos obsoleto se borran los metadatos de la última sync válida. Fix: `clear_stale_shopify_sync_errors` ahora hace un UPDATE (merge) de los 6 campos de error sobre el dict `shopifySync` existente (vía `config_store.update()`), preservando lastSync/counts/status/etc. Regression tests: `ClearStaleShopifyErrorsTests` (2 tests: preserva metadatos al limpiar errores; no toca si no hay errores). Suite: 707 passed. |
| BUG-024 | MEDIUM | Security | 3.0.8 | FIXED → READY FOR RETEST | `desktop/runtime/config_store.py` | `tests/test_config_store.py::Bug024CredentialsAtomicTests` | `config_store.secure_store_credentials` (config_store.py:304-313) hace read-modify-write sobre `credentials.json` (archivo de API keys) SIN lock, a diferencia de `maios.json` que sí usa `_config_lock`. Lee el archivo → añade la credencial del provider → reescribe el archivo completo. Llamado desde `ai_providers.save_provider_config` (ai_providers.py:70), expuesto en `POST /api/ai/configure` (api_server.py:376, ThreadingHTTPServer). Dos configuraciones concurrentes de providers distintos pueden hacer lost-update: la credencial del provider que escribe primero se pierde si el otro leyó antes de que el primero escribiera. Mismo patrón lost-update de BUG-006/015/019/023 pero sobre el archivo de credenciales (pérdida de API key). Fix: `_credentials_lock` dedicado serializa el RMW completo de `credentials.json` (read→modify→write bajo un solo lock). Regression test: `Bug024CredentialsAtomicTests` (dos providers persisten ambos). Suite: 708 passed. |
