...
function getVersion() {
  try {
    const vf = isDev
      ...
      : path.join(process.resourcesPath, 'maios', 'version.json');
    if (fs.existsSync(vf)) {
...
function startRuntime() {
  ...
  const py = getPython();
  if (!py) {
    logElectronEvent('STARTING_RUNTIME', { status: 'failed', error_code: 'PYTHON_RUNTIME_MISSING' });
    return false;
  }
...
async function ensureRuntimeStarted() {
  if (await probeRuntimeHealth(2000)) {
    logElectronEvent('Runtime already healthy — skipping spawn');
    return true;
  }
  ...
  for (let attempt = 1; attempt <= RUNTIME_START_ATTEMPTS; attempt++) {
    ...
    logElectronEvent('Runtime API did not respond', { attempt });
    stopRuntimeProcess();
    await sleep(1500);
  }
...
async function boot() {
  // Rebrand migration: copy any legacy VANOVA user data into VANOVA before
